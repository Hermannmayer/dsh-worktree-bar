/**
 * Reusable, context-free Git layer for the dsh-worktree-bar bundle.
 *
 * Nothing in this module knows about Cordis, the Harness, sessions, or HTTP: it
 * takes plain paths and returns plain JSON. The Host row (`./plugin.js`) wraps it
 * with routes, and another plugin may import it directly:
 *
 * ```js
 * import { repoInfo, createWorktree } from 'dsh-worktree-bar/git'
 * ```
 *
...
/**
 * Keep linked paths out of the worktree's own status.
 *
 * A path the convention connected to the main checkout is not part of this
 * checkout's git content, but git does not agree on every platform: on POSIX a
 * directory symlink is a *file* to git, so a project rule like `database/` (a
 * directory-only pattern) does not match it. The link then shows as untracked,
 * which also makes `git worktree remove` refuse a worktree nobody dirtied.
 *
 * The exclude file git actually consults is the right home for that fact: it is
 * never committed and never edits the project's `.gitignore`. `--git-path`
 * names it for this checkout — for a linked worktree that is the COMMON
 * `.git/info/exclude`, not the per-worktree git directory, which git ignores
 * for this purpose. Patterns are anchored (`/name`) and carry no trailing slash,
 * because the anchored form matches a file, a directory and a symlink alike —
 * exactly the case the project's own rules cannot express.
 *
 * A path that is tracked in this checkout is skipped: excluding it would hide
 * real changes, and linking a tracked path is the project's mistake to see.
 *
 * @param {string} worktreePath - the checkout whose status must stay clean.
 * @param {readonly string[]} relativePaths - connected paths, checkout-relative.
 * @returns {Promise<{ written: boolean, patterns: string[], skippedTracked: string[] }>} what was excluded.
 */
export async function excludeLinkedPaths(worktreePath, relativePaths) {
  const patterns = []
  ...
  for (const entry of relativePaths) {
    ...
    let tracked = false
    try {
      await runGit(worktreePath, ['ls-files', '--error-unmatch', '--', relativePath])
      tracked = true
    } catch {
      tracked = false
    }
    if (tracked) {
      skippedTracked.push(relativePath)
      continue
    }
    patterns.push(`/${relativePath}`)
  }
  if (patterns.length === 0) return { written: false, patterns, skippedTracked }

  const reported = (await runGit(worktreePath, ['rev-parse', '--git-path', 'info/exclude'])).trim()
  const file = isAbsolute(reported) ? reported : resolve(worktreePath, reported)
  let current = ''
  try {
    current = await readFile(file, 'utf8')
  } catch {
  ...
  if (missing.length === 0) return { written: false, patterns, skippedTracked }
  await mkdir(dirname(file), { recursive: true })
  const prefix = current === '' || current.endsWith('\n') ? '' : '\n'
  await appendFile(file, `${prefix}# dsh-worktree: paths connected to the main checkout\n${missing.join('\n')}\n`, 'utf8')
  return { written: true, patterns, skippedTracked, file }
}
...
export async function createWorktree(request) {
  ...
  if (request.seed !== false) {
    try {
      ...
      // A connected path is not this checkout's content, but git only agrees on
      // Windows (where a junction is skipped as a directory). Excluding it in the
      // worktree keeps its status honest and keeps removal from refusing a
      // worktree nobody actually dirtied.
      const connected = [...seeded.linked, ...seeded.copied]
      if (connected.length > 0) {
        try {
          const exclusion = await excludeLinkedPaths(path, connected)
          seeded.excluded = exclusion.patterns
          seeded.trackedLinked = exclusion.skippedTracked
        } catch {
          // Best effort: a status wart must not fail a creation either.
        }
      }
    } catch (error) {
      seeded = {
